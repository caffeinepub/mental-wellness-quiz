...
import type { Attempt as _Attempt, Quiz as _Quiz } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async getAllAttempts(): Promise<Array<[Principal, Array<Attempt>]>> {
        if (this.processError) {
            try {
    ...
    async getMyAttempts(): Promise<Array<Attempt> | null> {
        if (this.processError) {
            try {
    ...
    async getUserAttempts(arg0: Principal): Promise<Array<Attempt> | null> {
        if (this.processError) {
            try {
    ...
    async submitAnswers(arg0: Array<bigint>): Promise<bigint> {
        if (this.processError) {
            try {
...
function from_candid_opt_n1(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [Array<_Attempt>]): Array<Attempt> | null {
    return value.length === 0 ? null : value[0];
}
function from_candid_opt_n2(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: [] | [_Quiz]): Quiz | null {
    return value.length === 0 ? null : value[0];
}
